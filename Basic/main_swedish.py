...
if __name__ == '__main__':
    ...
    barn = np.arange(1,19)
    cubic = np.arange(3, 13)
    feed = np.arange(1,3)
    alley = np.arange(13, 19)

    tic1 = time.time()
    data = readFAfile('FA_20191116T000000UTC.csv')
    ...
    print("Divide group: %s " % (time.time() - tic2))
    ...
    data.loc[data['x'] > 3339, 'x'] = 3339
    data.loc[data['y'] > 8737, 'y'] = 8737
    data.loc[data['y'] < 969, 'y'] = 969
    ...
    for i in range(len(Pair)):
        ...
        if in_range.sum() >= realContactThreshold: 
            ...
            alley_contacts = np.concatenate(all_contacts[alley - 1])
            ...
            for j in alley:
                alley_area = np.concatenate([alley_area, j*np.ones(len(all_contacts[j-1]))])
            ...
            if(len(alley_contacts) >= realContactThreshold):
                real_contact_list_alley[i] = np.empty([len(alley_contacts), 7], dtype = float)
                real_contact_list_alley[i][:, 0] = np.reshape(alley_contacts, [len(alley_contacts), ])
                real_contact_list_alley[i][:, 1] = np.reshape(x1.T[alley_contacts], [len(alley_contacts), ])
                real_contact_list_alley[i][:, 2] = np.reshape(y1.T[alley_contacts], [len(alley_contacts), ])
                real_contact_list_alley[i][:, 3] = np.reshape(x2.T[alley_contacts], [len(alley_contacts), ])
                real_contact_list_alley[i][:, 4] = np.reshape(y2.T[alley_contacts], [len(alley_contacts), ])
                real_contact_list_alley[i][:, 5] = np.reshape(dist.T[alley_contacts], [len(alley_contacts), ])
                real_contact_list_alley[i][:, 6] = np.reshape(alley_area, [len(alley_contacts), ])
    ...
    duration_alley = np.zeros(len(IDlist), dtype = int)
    friends_alley = np.zeros(len(IDlist), dtype = int)
    for i in np.arange(0, len(Pair)):
        ...
        duration_alley[i1] += len(real_contact_list_alley[i]) if real_contact_list_alley[i] is not None else 0
        duration_alley[i2] += len(real_contact_list_alley[i]) if real_contact_list_alley[i] is not None else 0
        if real_contact_list_alley[i] is not None:
            if len(real_contact_list_alley[i]) > 0:
                friends_alley[i1] += 1
                friends_alley[i2] += 1

    contact_table = np.reshape(np.concatenate([IDlist, duration_cubic, friends_cubic, duration_feed, friends_feed, duration_alley, friends_alley]), [len(IDlist), 7], order = 'F')
    print("Contact calculations: %s" % (time.time() - tic2))
    
    np.savetxt("contact_table.csv", contact_table, delimiter=',')
    with open('data.pkl', 'wb') as f:
        pickle.dump([real_contact_list_cubic, real_contact_list_feed, real_contact_list_alley], f, protocol = -1)
